...
const SEARCH_ENGINE_ID = '22381be05dee949d5';
const API_KEY = 'AIzaSyCDGmNEtDYFfG7NuM_zEQVLdYhAaOs_4kY';
...
module.exports = {
    ...
    obterImagens: async (pesquisaTexto, qtdFotos = 1) => {
        try {
            const resultados = await customSearch.cse.list({
                q: pesquisaTexto,
                cx: SEARCH_ENGINE_ID,
                num: qtdFotos,
                searchType: 'image',
            ...
            if (!resultados.data.items || resultados.data.items.length === 0) {
                throw new Error('Nenhuma imagem encontrada para o tema: ' + pesquisaTexto);
            }
    
            const imagens = resultados.data.items.map(item => item.link);
            return imagens;
        } catch (err) {
            throw err;
        }
